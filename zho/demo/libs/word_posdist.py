...
def worker(token_filename, posdist_path, wordlist, sentlens):
    token_filename = Path(token_filename)
    posdist_path = Path(posdist_path)
    posdist_filename = posdist_path.joinpath(token_filename.name+'.posdist')
    cache_result_dict = create_nested_dict([wordlist, sentlens])
        ...
        for line in lines:
            if len(line.split()) >= min(sentlens) and len(line.split()) <= max(sentlens):
                for word in wordlist:
                    cache_result_dict[word][len(line.split())] += [pos+1 for pos, sent_word in enumerate(line.split()) if sent_word==word]
...
def main(token_path, posdist_path, wordlist_filename, n_worker, min_sentlen, max_sentlen):
    ...
    # 为子进程准备参数
    token_filenames = glob.glob(str(Path(token_path).joinpath(f'*.{"token"}').absolute()))
