...
for word in words: # 每种语言的一个单词
	...
	for year in range(4):
		...
		axes[word_cnt].set_xlabel('目标词句中线性位置', fontsize=14)
		axes[word_cnt].set_ylabel('概率', fontsize=14)
		year_cnt += 1
	axes[word_cnt].legend(title='年', fontsize=14, title_fontsize=14)
	...
	word_cnt += 1
axes[2].legend(title='年', fontsize=14, title_fontsize=14, loc=(0.35,0.02))
fig.subplots_adjust(wspace=0.2)
plt.savefig('../images/year_stable_cn.png',format='png')
plt.close()
